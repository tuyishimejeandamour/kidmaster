...
export const InputModal: React.FC<{ action: ConfirmAction }> = React.memo((props) => {
    ...
    return (
        <div className="relative">
            <input type="text" onChange={handleChange} onKeyDown={(e) => e.stopPropagation()} value={confirmValue}
                   className="h-14 w-96 pl-10 pr-20 rounded-lg z-0 focus:shadow focus:outline-none"
                   placeholder="Enter name..."/>
            <div className="absolute top-2 right-2">
                <button onClick={handleCreate}
                        className="h-10 w-20 text-white rounded-lg bg-[#20232a] hover:bg-[#20232a]">Create
                </button>
            </div>
        </div>
    )
