...
export default function Selected() {
  ...
  const { selectedNodeIds,setShowSideEditor } = useUIStore();
      ...
      <div className='w-full'>
        <div className="px-4 ">
          <div className="py-3 flex items-center justify-start gap-2 ">
            <span onClick={() => setShowSideEditor(false)} className='rounded p-[2px] cursor-pointer bg-gray-700/40'><svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4  text-gray-600" fill="currentcolor" viewBox="0 0 24 24" width="24" height="24"><path fill="none" d="M0 0h24v24H0z" /><path d="M10.828 12l4.95 4.95-1.414 1.414L8 12l6.364-6.364 1.414 1.414z" /></svg></span>
            <span className="text-gray-800 font-semibold capitalize truncate">{node?.name}</span>
          </div>
        </div>
      </div>
      {ui}
