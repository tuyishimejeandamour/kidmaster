...
export default function SidebarHeader() {
    ...
    return (
        <div
            className="w-[calc(100%-20px)] sticky top-0 border-b border-b-slate-900/20  py-1 mb-1 pb-1 backdrop-blur-md flex h-8 flex-1 items-center justify-between z-20">
            <div>
                <ul className="grid grid-flow-col text-center text-gray-400  rounded-lg p-1">
                    <li key={'all'} onClick={() => setActiveCategory('all')}><span
                        className={`transition-all duration-300 ease-in-out flex text-center hover:cursor-pointer text-[11px] justify-center  ${activeCategory == 'all' ? 'backdrop-blur-md bg-white/5 rounded-lg  shadow uppercase text-slate-50 ' : ''} px-2 py-[2px]`}>ALL</span>
                    </li>

                    {
                        entries(groupBy(matchedNode, 'category')).map(([key, value]) => (

                            <li key={key} onClick={() => setActiveCategory(key)}><span
                                className={`transition-all duration-300 ease-in-out text-[11px] flex text-center hover:cursor-pointer justify-center uppercase ${activeCategory == key ? 'backdrop-blur-md rounded-lg shadow bg-white/5 text-slate-50' : ''} px-2 py-[2px]`}>{key}</span>
                            </li>
                        ))
                    }
                </ul>
            ...
            <div className="flex">
                <div className="relative text-gray-600 focus-within:text-gray-400">
                    <span className="absolute inset-y-1 left-1 flex items-center pl-0">
                    {
                        groupCommand != "action" ?
                            ...
                            (
                                <svg className="h-4 w-4 fill-white" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <input type="text" name="q"
                           className="py-[2px] text-sm text-white bg-gray-900/30 rounded-md pl-8 focus:outline-none"
                ...
                <div className="pl-3 hover:cursor-pointer w-6 h-full">
                    <svg xmlns="http://www.w3.org/2000/svg" className="fill-white/50 w-6 h-6" viewBox="0 0 24 24">
                        <path
