    ...
    return (
      ...
      >
        {isEditing ? (
          ...
          <div className='flex w-full items-center border border-transparent hover:border-gray-400 hover:rounded-sm cursor-pointer' onClick={() => setIsEditing(true)}>
            <span className='-ml-px pl-1  text-gray-700 flex align-top h-auto flex-1 overflow-y-auto scroll-design leading-[32px] text-[12px] max-h-[250px]'>{value || "No value yet"}</span>
            <span className='w-6 h-8 flex items-center'>
              <span className='w-4 h-4  flex items-center hover:bg-slate-400 rounded-sm'>
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-black" fill="currentcolor" viewBox="0 0 24 24" width="24" height="24"><path fill="none" d="M0 0h24v24H0z" /><path d="M5 11h14v2H5z" /></svg>
              </span>
            </span>
          </div>
        )}
