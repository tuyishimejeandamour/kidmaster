                ...
                <div className=" pb-3 ">
                    ...
                    <div className="w-full h-8 flex px-4 justify-between mb-1" >
                        <label className="flex w-1/3 justify-start " aria-label="X">
                            <span className="-ml-px  text-gray-200 block align-top h-8 w-8 flex-[32px] leading-[32px] text-[11px] ">background</span>
                        </label>
                        <label className="flex flex-1   border border-transparent  hover:border-gray-400 hover:rounded-sm" aw-ria-label="Y">
                            <span className="-ml-px flex  text-gray-200 justify-center  items-center h-8 w-8 flex-[32px] leading-[32px] text-[11px] text-center ">
                                <span className='bg-slate-100 w-5 h-5 inline-block rounded-sm cursor-pointer' />
                            </span>

                            <input autoComplete="new-password" spellCheck="false" onChange={(e) => onChange(e.target.value, "bg")} className=" pl-0 input-hover bg-transparent text-gray-200 text-[11px] ml-0 border-l-0 rounded-sm cursor-default bg-clip-padding w-full p-0 h-7 mt-[1px]" dir="auto" defaultValue={props.definition?.ui?.backgroundColor} />
                        </label>
                    </div>
                    {
                        ...
                        <div className="w-full h-8 flex px-4 justify-between mb-2">
                            <label className="flex w-1/3 justify-start " aria-label="Width">
                                <span className="-ml-px  text-gray-200 block align-top h-8 w-8 flex-[32px] leading-[32px] text-[11px]">header</span>
                            </label>
                            <label className="flex flex-1 border border-transparent  hover:border-gray-400 hover:rounded-sm" aria-label="Height">
                                <span className="-ml-px flex  text-gray-200 justify-center  items-center align-top h-8 w-8  leading-[32px]">
                                <span className='bg-slate-100 w-5 h-5 inline-block rounded-sm cursor-pointer' />
                                </span>
                                <input autoComplete="new-password" spellCheck="false" onChange={(e) => onChange(e.target.value, "h")} className=" pl-0 input-hover text-gray-200 text-[11px] ml-0 border-l-0 rounded-sm cursor-default bg-transparent bg-clip-padding w-full p-0 h-7 mt-[1px]" data-testid="transform-height" dir="auto" value={props.definition?.ui?.headerColor} />
                            </label>
