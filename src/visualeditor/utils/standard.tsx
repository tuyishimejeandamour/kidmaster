...
import {CodeNodeDefinition, CodeNodePinDefinition} from '../store/node';
import {DEFAULT_NODE_WIDTH, STANDARD_PIN_EXEC_IN, STANDARD_PIN_EXEC_OUT} from './consts';
import {buildNodeHeight, buildPinPosX, buildPinPosY} from './size-helper';
import {TextAreaInputPreset, TextAreaInputPresetProps,} from '../nodes/components/preset/TextAreaInputPreset';
...
export const renderConnected = (node: BaseInputPresetProps) => {
    ...
    return (
        <div className='w-full px-4'>
            <div className='flex w-full items-center border border-blue-400 rounded-lg cursor-pointer'
                 onClick={() => handleClick()}>
                <span
                    className='-ml-px pl-4  text-gray-700 flex align-top h-auto flex-1 overflow-y-auto scroll-design leading-[32px] text-[12px] max-h-[250px]'>{"Connected"}</span>
          ...
          <span className='w-4 h-4  flex items-center hover:bg-red-500 rounded-sm'>
            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-black hover:text-white" fill="currentcolor"
